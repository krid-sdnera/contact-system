...
yarn install
yarn run openapi-generator generate -i common/contact-system-api.yaml -g php-symfony -o api/lib/ContactSystem/Api --git-user-id contact-system --git-repo-id api --additional-properties=phpLegacySupport=false,composerProjectName=contact-system,composerVendorName=api
