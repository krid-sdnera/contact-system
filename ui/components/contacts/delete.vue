...
const { useDeleteContact } = useContact();
...
async function submitDelete() {
  try {
    const contactId = await deleteFn(props.contact.id);

    if (contactId) {
      emit('deleted', contactId);
    }
...
<template>
  <v-dialog v-model="model" persistent scrollable max-width="600px">
    <v-card style="height: 90vh">
      ...
      >
        <div>Delete {{ props.contact.firstname }}?</div>
        <v-btn icon="mdi-close" variant="text" @click="close"></v-btn>
      </v-card-title>

      <v-card-text>
        You are about to delete "{{ props.contact.firstname }}"<br />
